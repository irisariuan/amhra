...
app.post(
	...
	(req, res) => {
		const formatter = misc.prefixFormatter(chalk.bgGrey(`(Guild ID: ${req.body.guildId}, IP: ${req.ip})`))
		const cLog = (...data) => { log(formatter(data.join())) }
		const cError = (...data) => { error(formatter(data.join())) }
