...
async function init(client) {
	 ...
	 * @param {{requirePassword: boolean, allowBearer: boolean}} setting 
	 * @returns {(req: express.Request, res: express.Response, next: express.NextFunction) => void}
	 */
	function auth(setting = { requirePassword: true, allowBearer: false }) {
		return async (req, res, next) => {
			...
			if (setting.allowBearer && req.headers.authorization.startsWith('Bearer')) {
				if (await countUser(removeBearer(req.headers.authorization)) > 0) {
					return next()
				}
				exp.error(formatter("Auth failed (NOT_MATCHING_DB)"))
			}
			if (!setting.requirePassword && !req.headers.authorization.startsWith('Basic')) {
				return next()
			}
			if (setting.requirePassword && !req.headers.authorization.startsWith("Basic") && !req.headers.authorization.startsWith("Bearer")) {
