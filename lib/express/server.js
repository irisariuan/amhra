...
async function init(client) {
	function auth(password = true, bearer = false) {
		return async (req, res, next) => {
			...
			if (!password && !req.headers.authorization.startsWith('Basic')) {
				return next()
			}
			if (bearer && req.headers.authorization.startsWith('Bearer')) {
				if (await countUser(removeBearer(req.headers.authorization)) > 0) {
					return next()
				}
				exp.error(formatter("Auth failed (NOT_MATCHING_DB)"))
			}
			if (password && !req.headers.authorization.startsWith("Basic") || !req.headers.authorization.startsWith("Bearer")) {
	...
	app.get("/api/new", auth(true, true), (req, res) => {
		...
		(req, res) => {
			...
			switch (req.body.action) {
				...
				case 'setVolume': {
					...
					event.emit('songInterruption', req.body.guildId, req.body.action, volume)
					break
				}
				case 'setQueue': {
	...
	app.post('/api/videoSuggestion', jsonParser, auth(false), basicCheckBuilder(['query']), async (req, res) => {
	...
	app.get("/api/logout", auth(true, true), async (req, res) => {
		exp.log(`${req.ip} just logout`)
		res.sendStatus(200)
	})

	app.get("/api/playingGuildIds", auth(false, true), async (req, res) => {
	...
	app.get("/api/song/get/:guildId", auth(false, true), (req, res) => {
