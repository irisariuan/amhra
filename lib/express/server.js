 ...
 * @returns {[express.Express, Map<string, {guilds: string[], level: number}>, (string) => {token: string, guilds: string[], level: number}]}>
 */
function init(client) {
	...
	function registered(protective = true) {
		return (req, res, next) => {
			if (!protective && !req.headers.authorization.startsWith('Basic')) {
				return next()
			}
	...
	/**
	 * 
	 * @param {string} guildId Guild IDs
	 * @returns {{token: string, guilds: string[], level: number}}
	 */
	function newToken(guildId) {
		const token = generateToken(36)
		const level = 1
		authLevel.set(token, { guilds: [guildId], level })
	...
	function checkGuild(auth, guildId = '') {
		if (auth.startsWith('Basic') || authLevel.get(auth)?.guilds?.includes(guildId)) {
			return true
		}
		return false
	}
		...
		(req, res) => {
			...
			switch (req.body.action) {
				...
				case "addAuth": {
					...
					const { token, level } = newToken([guildId])
					exp.log('Successfully created server-based dashboard')
	...
	app.post('/api/getVideoDetail', jsonParser, auth(false), registered(false), basicCheckBuilder(['url']), async (req, res) => {
		if (!req.body.url || !YoutubeVideoRegex.test(req.body.url)) {
			return res.sendStatus(400)
		}
		try {

			const video = await video_info(req.body.url)
			if (!video) {
	...
	app.get("/api/song/get/:guildId", auth(false), registered(false), (req, res) => {
