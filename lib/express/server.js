...
function init(client) {
	...
	function checkGuild(req, res, next) {
		if (authLevel.get(req.headers.authorization)?.guilds?.includes(req.body.guildId)) {
			return next()
		}
		res.sendStatus(401)
	}
		...
		(req, res) => {
			...
			switch (req.body.action) {
				...
				case "addAuth": {
					...
					console.log('Successfully created server-based dashboard')
	...
	app.post('/api/getVideoDetail', jsonParser, auth, registered, basicCheckBuilder(['url']), async (req, res) => {
	...
	app.post("/api/live", auth, registered, basicCheckBuilder(['guildId']), checkGuild, async (req, res) => {
		return res.sendStatus(200)
	})
