...
async function init(client) {
	 ...
	 * @param {{requirePassword: boolean, allowBearer: boolean}} param0 
	 * @returns {(req: express.Request, res: express.Response, next: express.NextFunction) => void}
	 */
	function auth({ requirePassword = true, allowBearer = false }) {
		return async (req, res, next) => {
			...
			if (!requirePassword && !req.headers.authorization.startsWith('Basic')) {
				return next()
			}
			if (requirePassword && !req.headers.authorization.startsWith("Basic") && !req.headers.authorization.startsWith("Bearer")) {
