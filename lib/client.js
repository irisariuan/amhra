...
client.on("messageCreate", async message => {
	if (message.author.id !== client.user?.id) {
		dcb.messageLog(
			`${message.author.tag} (Guild ID: ${message.guildId}, Channel ID: ${message.channelId}, Message ID: ${message.id}): '${chalk.bgWhite.black(message.content)}'`
		)
	}
...
client.on('messageDelete', message => {
	dcb.messageLog(`${chalk.red('[DELETE]')} ${message.author.tag} (Guild ID: ${message.guildId}, Channel ID: ${message.channelId}, Message ID: ${message.id}) deleted '${chalk.bgWhite.black(message.content)}'`)
})
