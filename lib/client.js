...
client.on("messageCreate", async message => {
	...
	if (message.author.id !== client.user?.id) {
		dcb.messageLog(
			`${message.author.tag} (Guild ID: ${message.guildId}, Channel ID: ${message.channelId}, Time: ${Date.now()}): ${msg}`
		)
	}
...
client.on('voiceStateUpdate', (oldState, newState) => {
	const formatter = misc.prefixFormatter(`${chalk.bgMagentaBright('[VOICE]')} (Channel ID: ${newState.channelId}, Guild ID: ${newState.guild.id})`)
