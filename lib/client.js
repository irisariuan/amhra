...
const { app, registered } = require("./express/server.js")
...
client.on("messageCreate", async message => {
	...
	dcb.messageLog(
		`${message.author.tag} (Guild ID: ${message.guildId}, Channel ID: ${message.channelId}, Message ID: ${message.id}): '${chalk.bgWhite.black(message.content)}'${message.attachments ? `Attachments: ${message.attachments.map(v => `URL: ${v.url}, Type: ${v.contentType}`).join(', ')}` : ''}`
	)
