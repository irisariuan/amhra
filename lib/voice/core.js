...
function createAudioPlayer(guildId, createOpts = {}) {
	...
	event.on("songDataRequest", (id) => {
		if (id !== guildId) return
		event.emit("songDataReply", {
			song: player.nowPlaying
				? {
						link: player.nowPlaying.url,
						channel: player.nowPlaying.channel.url,
						duration: player.nowPlaying.details.durationInSec,
						title: player.nowPlaying.details.title,
						thumbnails: player.nowPlaying.details.thumbnails.map((v) => v.url),
				  }
				: null,
			queue: player.queue,
