...
async function createStream(url, seek = undefined) {
	...
	streamCache.set(`${seek};${url}`, source)
	return source
}
 ...
 */
async function createResource(url, seek = undefined, useCache = true) {
	const source = (useCache && streamCache.get(`${seek};${url}`)) || await createStream(url, seek)
	let videoInfo
	if (videoInfoCache.get(url)) {
		videoInfo = videoInfoCache.get(url)
	} else {
