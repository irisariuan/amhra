 ...
 */
function createStream(url, seek = undefined) {
	const source = stream(url, {seek})
	streamCache.set({ url, seek: seek ?? 0 }, source)
	return source
}
...
async function createResource(url, seek = undefined) {
	const source = streamCache.get({ url, seek: seek ?? 0 }) ?? createStream(url, seek)
	let videoInfo
